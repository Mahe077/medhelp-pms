...
import com.medhelp.pms.modules.auth_module.application.dtos.AuditLogDto;
...
public interface SettingsAuditRepository extends JpaRepository<DomainEventEntity, Long> {
    ...
    @Query("""
            SELECT new com.medhelp.pms.modules.auth_module.application.dtos.AuditLogDto(
                de.eventId, de.eventType, de.aggregateType, de.aggregateId,
                CAST(de.eventData AS string), de.occurredAt, de.userId,
                CONCAT(u.firstName, ' ', u.lastName), de.sequenceNumber
            )
